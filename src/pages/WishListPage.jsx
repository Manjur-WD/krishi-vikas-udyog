    ...
    return (
        <>
            ...
            <main>
                ...
                <section className="wishlist-section grid grid-cols-1 md:grid-cols-3 2xl:grid-cols-5 md:px-5  px-2 md:gap-x-4 gap-x-2 container my-5">
                    {
                                    ...
                                    <div className="wishlist-desc truncate p-1 pe-0">
                                        ...
                                        <div className="flex items-center justify-between wishlist-price-location text-sm p-2 text-white mt-1 rounded-xl">
                                            ...
                                            <p className="">
                                                ...
                                                {item?.price}
                                                {item?.rent_type !== null ? `/${item?.rent_type.slice(4)}` : ""}
                                            </p>
                                        </div>
                                        <div className="wishlist-action flex justify-center gap-3 px-5 mt-2">
                                            <button type="button" className="bg-lightdark text-white md:text-lg text-sm px-2 py-1 rounded-3xl shadow active:scale-95"><FaRegEye className="inline mb-1" /> View</button>
                                            <button type="button" className="bg-red-700 text-white md:text-lg text-sm px-2 py-1 rounded-3xl shadow active:scale-95" onClick={() => { handleRemove(item?.category_id, item?.id) }}><CiTrash className="inline mb-1" />Remove</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
